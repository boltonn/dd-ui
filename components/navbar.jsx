    ...
    return navbarItems?.length ? (
        <nav className='flex items-center justify-center px-4 mx-auto overflow-x-auto text-l whitespace-nowrap'>
            {navbarItems.map((item, index) => (
                <Link
                    key={index}
                    className={`hover:cursor-pointer hover:text-violet-500 dark:hover:text-violet-500 flex flex-col items-center justify-center w-16 h-16 group ${pathname === item.href ? 'text-violet-700 ' : 'dark:text-gray-200'}`}
                    href={item.href}
                >
